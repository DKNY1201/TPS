...
<form method="post" action="">
    <table class="table table-bordered dTableR">
        <thead>
            <th colspan="2" class="header-table">Chỉnh sửa hình ảnh</th>
        </thead>
        <tr>
            <td>Tên VN</td>
        ...
        <tr>
            	<td>Loại hình ảnh</td>
                <td>
                	<select name="images_type">
                    <? 
						while($row_it=mysql_fetch_assoc($it)) {
							$idit=$row_cr['idImgType'];
							$s="";
							if($idit==$row_pt['idImgType'])
								$s="selected=selected";
					?>
                    	<option value="<?=$row_it['idImgType']?>" <?=$s?>><?=$row_it['name_vn']?></option>
                    <? }?>
                    </select>
                </td>
            </tr>
