...
<a href="index.php?p=hr_add" class="btn btn-danger btn-add" title="Thêm bài viết Nhân sự">Thêm bài viết</a>
    ...
    <tbody>
     ...
     <? while($row_pt=mysql_fetch_assoc($pt)){
      ...
      <tr>
        ...
        <td><a class="icon-edit" title="Chỉnh sửa" href="index.php?p=hr_edit&idHR={ID}"></a> <? if($_SESSION['level']==1){?><a onclick="return confirm('Bạn muốn xóa bài viết {TenVN}?')" class="icon-remove" title="Xóa" href="hr_del.php?idHR={ID}"></a><? }?></td>
      ...
      <?
        ...
        $str=str_replace("{ID}",$row_pt['idHR'],$str);
