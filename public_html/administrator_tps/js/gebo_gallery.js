/* [ ---- Gebo Admin Panel - gallery grid ---- ] */
    ...
    gebo_gal_grid = {
        ...
        large: function() {
            $('.image-gallery ul').imagesLoaded(function() {
                // Prepare layout options.
                var options = {
                  autoResize: true, // This will auto-update the layout when the browser window is resized.
                  container: $('.image-gallery'), // Optional, used for some extra CSS styling
                ...
                var handler = $('.image-gallery ul li');
                
                // Call the layout function.
                handler.wookmark(options);
                
                
                $('.image-gallery ul li').on('mouseenter',function(){
                    $(this).addClass('act_tools');	
                }).on('mouseleave',function(){
                    $(this).removeClass('act_tools');
                });
                
                $('.image-gallery ul li > a').attr('rel', 'gallery').colorbox({
                    maxWidth	: '80%',
                    maxHeight	: '80%',
                    opacity		: '0.1', 
                    loop		: true,
